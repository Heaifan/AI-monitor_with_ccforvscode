...
const formatTime = (ts) => {
    const d = new Date(ts); return `${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`;
};

module.exports = {
    markStart: (ts) => { currentLog = { rawStart: ts, start: formatTime(ts) }; },
